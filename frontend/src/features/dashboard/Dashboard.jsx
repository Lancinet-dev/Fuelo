...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: theme.colors.card, border: `1px solid ${theme.colors.cardBorder}`, borderRadius: theme.radius.md, padding: '10px 14px', boxShadow: theme.shadow.md }}>
      <div style={{ fontSize: theme.font.size.xs, color: theme.colors.textSub, marginBottom: 4 }}>{label}</div>
...
// ── Ligne vente récente ───────────────────────────────
function VenteRow({ vente, isLast }) {
  return (
    <div
      style={{ display: 'grid', gridTemplateColumns: '36px 1fr 90px 110px 90px', alignItems: 'center', padding: '12px 20px', borderBottom: isLast ? 'none' : `1px solid ${theme.colors.cardBorder}`, transition: theme.transition.fast, gap: 8, cursor: 'default' }}
    ...
    >
      ...
      <div>
        <div style={{ fontSize: theme.font.size.md, fontWeight: theme.font.weight.semi, color: theme.colors.text, textTransform: 'capitalize' }}>{vente.type}</div>
        <div style={{ fontSize: theme.font.size.xs, color: theme.colors.textMuted }}>{vente.employe_nom ?? 'Pompiste'}</div>
      </div>
    ...
    <div style={{ padding: '32px 28px', maxWidth: 1200, margin: '0 auto' }} className="fuelo-dashboard">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button
            onClick={refetch}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 16px', borderRadius: theme.radius.md, border: `1px solid ${theme.colors.cardBorder}`, background: theme.colors.card, color: theme.colors.textSub, cursor: 'pointer', fontSize: theme.font.size.md, fontFamily: theme.font.family, boxShadow: theme.shadow.sm }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }} className="fuelo-grid-3">
        <StatCard
          ...
          sub={`${aujourdhui.nb} transactions · ${formatLitres(aujourdhui.litres)}`}
          icon={ICONS.ventes}
        ...
        <StatCard
          ...
          sub={`${cemois.nb} transactions · ${formatLitres(cemois.litres)}`}
          icon={ICONS.trend}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14, marginBottom: 14 }} className="fuelo-grid-2">
        ...
        {/* Graphique */}
        <div style={{ background: theme.colors.card, border: `1px solid ${theme.colors.cardBorder}`, borderRadius: theme.radius.lg, padding: '22px 24px', boxShadow: theme.shadow.sm }}>
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: theme.font.size.base, fontWeight: theme.font.weight.bold, color: theme.colors.text, marginBottom: 3 }}>Ventes — 7 derniers jours</div>
            <div style={{ fontSize: theme.font.size.xs, color: theme.colors.textSub }}>En millions GNF</div>
          </div>
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="jour" tick={{ fill: theme.colors.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: theme.colors.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
        ...
        {/* Ventes récentes */}
        <div style={{ background: theme.colors.card, border: `1px solid ${theme.colors.cardBorder}`, borderRadius: theme.radius.lg, boxShadow: theme.shadow.sm, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 20px', borderBottom: `1px solid ${theme.colors.cardBorder}` }}>
            <div style={{ fontSize: theme.font.size.base, fontWeight: theme.font.weight.bold, color: theme.colors.text }}>Ventes récentes</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr 90px 110px 90px', padding: '8px 20px', background: '#F9FAFB', borderBottom: `1px solid ${theme.colors.cardBorder}`, gap: 8 }}>
            {['', 'Type', 'Litres', 'Montant', 'Quand'].map(h => (
              <div key={h} style={{ fontSize: 10, fontWeight: 700, color: theme.colors.textMuted, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{h}</div>
            ))}
          </div>
          {recentes.length > 0
            ? recentes.map((v, i) => <VenteRow key={v.id} vente={v} isLast={i === recentes.length - 1} />)
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }} className="fuelo-grid-3">
        {[
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderRadius: theme.radius.lg, border: `1px solid ${theme.colors.cardBorder}`, background: theme.colors.card, cursor: 'pointer', fontFamily: theme.font.family, fontSize: theme.font.size.md, fontWeight: theme.font.weight.semi, color: theme.colors.text, boxShadow: theme.shadow.sm, transition: theme.transition.fast, textAlign: 'left', width: '100%' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = color; e.currentTarget.style.boxShadow = theme.shadow.md }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = theme.colors.cardBorder; e.currentTarget.style.boxShadow = theme.shadow.sm }}
          >
