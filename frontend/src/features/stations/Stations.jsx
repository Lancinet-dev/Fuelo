  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(6px)', padding: 24 }}>
      <div style={{ background: palette.card, border: `1px solid ${palette.cardBorder}`, borderRadius: theme.radius.xl, padding: '28px 32px', maxWidth: 520, width: '100%', boxShadow: theme.shadow.lg }}>
        <h2 style={{ fontSize: theme.font.size.xl, fontWeight: theme.font.weight.black, color: palette.text, letterSpacing: '-0.5px', marginBottom: 4 }}>{t('stations.nouvelle')}</h2>
        <p style={{ fontSize: theme.font.size.md, color: palette.textSub, marginBottom: 24 }}>{t('stations.modalSub')}</p>

        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 14 }}>
            {label(`${t('stations.nomLabel')} *`)}
          ...
          <div style={{ background: palette.hover, border: `1px solid ${palette.cardBorder}`, borderRadius: theme.radius.md, padding: '14px 16px', marginBottom: 22 }}>
            {label(t('stations.seuils'))}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <div style={{ fontSize: theme.font.size.xs, color: palette.textSub, marginBottom: 6 }}>{t('stock.essence')}</div>
              ...
              <div>
                <div style={{ fontSize: theme.font.size.xs, color: palette.textSub, marginBottom: 6 }}>{t('stock.gasoil')}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? t('stations.creation') : t('stations.creer')}
            </button>
          </div>
        </form>
      </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '12px 18px', borderBottom: `1px solid ${palette.cardBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontSize: theme.font.size.xs, color: palette.textSub }}>{t('stations.ventesAujourdhui')}</div>
      ...
      <div style={{ padding: '12px 18px' }}>
        ...
        >
          ...
          {switching ? t('stations.changement') : t('stations.basculer')}
        </button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ padding: '32px 28px', maxWidth: 1100, margin: '0 auto' }} className="fuelo-stations">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 14 }}>
        <div>
          <h1 style={{ fontSize: theme.font.size['2xl'], fontWeight: theme.font.weight.black, color: palette.text, letterSpacing: '-0.5px', margin: 0, marginBottom: 4 }}>{t('stations.title')}</h1>
          <p style={{ fontSize: theme.font.size.md, color: palette.textSub, margin: 0 }}>
            {stations.length} station{stations.length > 1 ? 's' : ''}{maxStations ? ` / ${maxStations} max` : ''} · {t('stations.vueProprietaire')}
          </p>
        </div>
        <button
          ...
          {limitAtteinte ? `${t('stations.limiteAtteinte')} (${maxStations} max)` : t('stations.nouvelle')}
        </button>
      </div>
      ...
      {!loading && stations.length > 1 && (
        <div style={{ background: `linear-gradient(135deg, rgba(37,99,235,0.08), rgba(37,99,235,0.03))`, border: `1px solid rgba(37,99,235,0.2)`, borderRadius: theme.radius.lg, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontSize: theme.font.size.xs, fontWeight: theme.font.weight.semi, color: theme.colors.primary, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 16 }}>
            {t('stations.vueConsolidee')}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0 }} className="fuelo-grid-3">
            {[
              { label: t('stations.stationsActives'), value: String(consolide.nb_stations ?? stations.length), color: theme.colors.primary },
              { label: t('stations.ventesJour'),      value: formatGNF(consolide.ventes_jour_total),           color: theme.colors.success },
              { label: t('stations.alertesTotales'),  value: String(consolide.alertes_total ?? 0),             color: parseInt(consolide.alertes_total) > 0 ? theme.colors.danger : theme.colors.success },
      ...
      {loading ? (
        ...
        <div style={{ background: palette.card, border: `1px solid ${palette.cardBorder}`, borderRadius: theme.radius.lg }}>
          <EmptyState type="stations" actionLabel={t('stations.creerPremiere')} onAction={() => setShowModal(true)} />
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }} className="fuelo-grid-2">
