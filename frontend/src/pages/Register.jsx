...
export default function Register() {
  ...
  const Field = ({ label, type = 'text', value, onChange, placeholder, suffix, extra }) => (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: t.textSub, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>
        {label}
      </div>
      <div style={{ position: 'relative' }}>
        <input
          ...
          style={{ width: '100%', height: 48, background: t.inputBg, border: `1.5px solid ${t.border}`, borderRadius: 11, padding: suffix ? '0 60px 0 16px' : '0 16px', fontSize: 14, color: t.text, fontFamily: 'inherit', outline: 'none', transition: 'all 0.25s' }}
      ...
      <div style={{ background: t.left, padding: '44px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: '#fff', lineHeight: 1.2, marginBottom: 10, letterSpacing: '-0.8px' }}>
            Rejoignez les<br /><span style={{ color: '#F59E0B' }}>200+ stations</span><br />qui utilisent Fuelo
          </div>
        ...
        <div style={{ width: '100%', maxWidth: 360 }}>
          ...
          {step === 1 && (
            <div>
              ...
              <Field label="Votre nom complet" value={form.nom} onChange={v => set('nom', v)} placeholder="Mamadou Diallo" />
              ...
              <div style={{ textAlign: 'center', fontSize: 13, color: t.textSub }}>
                Déjà un compte ?{' '}
                <span onClick={() => navigate('/login')} style={{ color: '#F59E0B', fontWeight: 600, cursor: 'pointer' }}>Se connecter</span>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              ...
              <Field label="Nom de la station" value={form.nom_station} onChange={v => set('nom_station', v)} placeholder="Ex: Station Almamya" />
              <Field label="Adresse (optionnel)" value={form.adresse} onChange={v => set('adresse', v)} placeholder="Ex: Rue KA-020, Kaloum" />
              <Field label="Ville" value={form.ville} onChange={v => set('ville', v)} placeholder="Conakry" />
              <Field label="Pays" value={form.pays} onChange={v => set('pays', v)} placeholder="Guinée" />
          ...
          {step === 3 && (
            <div>
              ...
              {/* Résumé */}
              <div style={{ background: t.card, border: `0.5px solid ${t.border}`, borderRadius: 12, padding: '18px 20px', marginBottom: 20 }}>
                ...
                {[
                  ...
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: `0.5px solid ${t.border}` }}>
